...
namespace MarginTrading.MarketMaker.Infrastructure.Implementation
{
    ...
    {
        public static CachedCalculation<TSource, TResult> Cached<TSource, TResult>(Func<TSource> source,
    ...
    ///     Represents a lazy function whose value will not be recalculated if source value is not changed.<br />
    ///     Caches last calculation execution result.<br />
    ///     Source function is executed on each act of obtaining the calculation result. It should represent a rarely changeable value.<br />
    ///     Instances are not very thread-safe, but at least it is always guaranteed not to mix up the results for different source values.
    /// </summary>
    public class CachedCalculation<TSource, TResult>
    {
        ...
        [CanBeNull]
        private volatile SourceResultPair _cache;
